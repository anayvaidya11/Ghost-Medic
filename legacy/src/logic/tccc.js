...
export function generateProtocol({ underFire, mechanism, symptoms, vitals, supplies }) {
  const has = (id) => symptoms.includes(id);
  const have = (id) => supplies.includes(id);
  const steps = [];
  ...
  if (underFire) {
    steps.push({
      ...
      detail: 'Treat only life-threatening extremity hemorrhage. Move casualty to cover. Full assessment follows in Tactical Field Care.',
      resource: null,
      supplyAlt: null,
      steps: [
        ...
        { text: 'Apply tourniquet to life-threatening extremity bleed ONLY — do not expose wound', critical: true },
  ...
  if (has('extremity_bleed')) {
    steps.push({
      ...
      detail: 'Extremity hemorrhage is the #1 preventable cause of death in combat. Tourniquet now — no hesitation.',
      resource: have('tourniquet') ? 'CAT / SOFTT-W Tourniquet' : 'Improvised tourniquet (belt or cord)',
      supplyAlt: !have('tourniquet') ? 'Belt or strip of cloth: tie around limb 2–3in above wound, insert stick, twist until bleeding stops, secure stick to limb.' : null,
      steps: [
      ...
      diagram: 'Tourniquet position: 2–3 inches ABOVE wound on extremity. Not over joint.',
  ...
  if (has('junctional_bleed') || has('neck_bleed') || has('groin_bleed') || has('armpit_bleed')) {
    steps.push({
      ...
      resource: have('hemostatic_gauze') ? 'Combat Gauze / QuikClot ACS+' : have('gauze') ? 'Gauze rolls' : 'Cleanest available cloth',
      supplyAlt: !have('hemostatic_gauze') ? 'Regular gauze or clean cloth — pack tightly into wound cavity, direct pressure minimum 3 minutes.' : null,
      steps: [
        ...
        { text: 'If still bleeding: add material on top — do NOT remove first pack', critical: true },
      ],
      diagram: 'Pack wound cavity tightly. Do not just cover surface. Push gauze deep to bleeding point.',
  ...
  if (has('heavy_bleeding') && !has('extremity_bleed') && !has('junctional_bleed')) {
    steps.push({
      ...
      steps: [
        ...
        { text: 'Limb wound? → Apply tourniquet 2–3in above wound immediately', critical: true },
  ...
  if (have('tranexamic') && (has('extremity_bleed') || has('junctional_bleed') || has('heavy_bleeding'))) {
    steps.push({
      id: 'txa',
      phaseCode: 'M',
      priority: 2,
  ...
  // ── A: AIRWAY ────────────────────────────────────────────────────
  const airwayIssue = has('unconscious') || has('gurgling') || has('airway_obstruction') || has('snoring_breathing');
  ...
  if (airwayIssue) {
    steps.push({
      ...
      supplyAlt: !have('npa') ? 'Recovery position (on side) maintains airway without equipment for unconscious casualty.' : null,
      steps: has('unconscious') ? [
        { text: 'Tilt head back, lift chin (unless spinal injury suspected → jaw thrust)', critical: true },
        { text: 'Look, listen, feel for breathing — 10 seconds', critical: true },
        { text: 'Gurgling? Sweep finger through mouth to clear blood/debris', critical: false },
        { text: 'Insert NPA: lubricate, insert into larger nostril bevel toward septum', critical: false },
        { text: 'Position on side (recovery position) if breathing but unconscious', critical: true },
      ...
      diagram: 'NPA insertion: right nostril first. Bevel toward nasal septum. 6cm depth typical adult.',
      diagnosis: has('unconscious') ? 'Unconscious casualty — airway management required' : 'Airway obstruction',
    });
  }
  ...
  if (has('sucking_chest_wound') || has('chest_wound')) {
    steps.push({
      ...
      detail: 'Open pneumothorax. Seal on exhalation to prevent tension pneumothorax. Use vented seal.',
      resource: have('chest_seal') ? 'Hyfin / Bolin vented chest seal' : 'Improvised 3-sided occlusive dressing (plastic + tape)',
      supplyAlt: !have('chest_seal') ? 'Cut plastic (MRE wrapper, IV bag). Cover wound. Tape 3 sides only — leave bottom edge open as flutter valve.' : null,
      steps: [
        ...
        { text: 'Apply chest seal over wound on exhalation — press ALL edges firmly', critical: true },
        { text: 'Check for EXIT wound on back — seal if found', critical: true },
        { text: 'If using improvised: tape 3 sides, leave bottom open as flutter valve', critical: false },
        { text: 'MONITOR: if breathing worsens or trachea deviates — burp the seal briefly', critical: true },
      ],
      diagram: 'Seal on exhalation. Vented seals preferred — allow trapped air to escape. Exit wound is as dangerous as entry.',
      diagnosis: 'Penetrating chest trauma — open pneumothorax',
    });
  }

  const tensionSigns = (vitals.oxygenSat && vitals.oxygenSat < 90) ||
    (vitals.heartRate && vitals.heartRate > 120) ||
    has('deviated_trachea') || has('neck_vein_distention') ||
  ...
  if (tensionSigns && (has('chest_wound') || has('sucking_chest_wound'))) {
    steps.push({
      ...
      detail: 'Tension pneumothorax: collapsed lung + mediastinal shift. Immediately fatal without decompression.',
      resource: have('needle_decompression') ? '14g 3.25in catheter-over-needle / NCD kit' : 'Largest available needle (14–16g preferred)',
      supplyAlt: null,
      steps: [
        ...
        { text: 'Insert needle perpendicular to chest wall — rush of air confirms tension', critical: true },
      ...
      diagram: 'Lateral 4th–5th ICS (preferred): follow armpit line down to nipple level, insert over rib top (avoid vessels under rib).',
  ...
  const shockSigns =
    ...
    (vitals.oxygenSat && vitals.oxygenSat < 94) ||
    has('pale_skin') || has('cool_clammy') || has('rapid_weak_pulse') || has('altered_mental_status');
  ...
  if (shockSigns) {
    steps.push({
      ...
      detail: 'Signs of shock. IV/IO access and controlled resuscitation. Target SBP 80–90 (permissive hypotension).',
      resource: have('iv_kit') ? 'Large-bore IV kit + fluids' : have('io_kit') ? 'IO drill kit (tibial)' : 'Positioning only',
      supplyAlt: !have('iv_kit') && !have('io_kit') ? 'No vascular access: lay flat, elevate legs 12in, keep warm, urgent MEDEVAC.' : null,
      steps: [
      ...
      diagram: 'Permissive hypotension: deliberately low BP slows re-bleeding from clot. Do not over-hydrate penetrating trauma.',
  ...
  // ── H: HYPOTHERMIA + HEAD ────────────────────────────────────────
  const hypothermiaSigns = (vitals.skinTemp && vitals.skinTemp < 35) || has('shivering') || has('cold_skin') || has('hypothermia');
  ...
  if (hypothermiaSigns || hasSignificantTrauma) {
    steps.push({
      ...
      detail: 'Hypothermia triad: coagulopathy + acidosis + hypothermia = death. Wrap every trauma patient.',
      resource: have('hypothermia_blanket') ? 'Hypothermia Prevention Kit / Ready-Heat blanket'
        : have('space_blanket') ? 'Space / emergency blanket'
      ...
      diagram: null,
      diagnosis: hypothermiaSigns ? 'Active hypothermia — rewarming required' : 'Hypothermia prevention — standard trauma protocol',
    });
  }
  ...
  if (has('head_injury') || has('loss_of_consciousness') || has('seizure')) {
    steps.push({
      ...
      detail: 'TBI: prevent secondary injury. Maintain airway and BP. Do NOT give opioids for isolated TBI.',
      resource: have('eye_shield') ? 'Eye shield (if eye injury)' : 'Monitoring + positioning',
      supplyAlt: null,
      steps: [
        { text: 'Maintain airway — unconscious TBI requires active airway management', critical: true },
        ...
        { text: 'Do NOT give morphine for isolated TBI — use ketamine if pain control needed', critical: true },
        { text: 'Monitor pupils every 5 min: unequal / blown pupil = herniation → urgent MEDEVAC', critical: true },
        { text: 'If eye injury: shield eye, do NOT rub or apply pressure to globe', critical: false },
        { text: 'Seizure: protect from injury, roll to side, do NOT restrain', critical: false },
      ],
      diagram: 'Pupil check: shine light in eye. Normal = constricts. Blown (dilated + non-reactive) = herniation. ACT NOW.',
      diagnosis: 'Traumatic brain injury — conservative management and monitoring',
    });
  }

  // ── PAIN ─────────────────────────────────────────────────────────
  const canGivePain = !has('head_injury') && !has('unconscious') && !has('loss_of_consciousness');
  ...
  if (canGivePain && hasSignificantTrauma) {
    steps.push({
      ...
      resource: have('ketamine') ? 'Ketamine (preferred TCCC analgesic)'
        : have('morphine') ? 'Morphine (with restrictions)'
        : have('ibuprofen') ? 'Meloxicam 15mg PO or Ibuprofen 800mg PO'
      ...
      steps: [
        ...
        have('ketamine')
          ...
          : have('morphine')
            ? { text: 'Morphine 5mg IV/IO: titrate slowly. Contraindicated: SBP<90, RR<12, TBI.', critical: true }
            : have('ibuprofen')
              ? { text: 'Meloxicam 15mg PO or Ibuprofen 800mg PO with food if able', critical: false }
              : { text: 'No meds: reassurance, comfortable positioning, explain each step', critical: false },
  ...
  steps.push({
    ...
    supplyAlt: null,
    steps: [
      { text: 'M — MECHANISM: describe how injury occurred (GSW, blast, fall…)', critical: false },
      ...
      { text: 'T — TREATMENT: tourniquet time, fluids given (volume), medications (name/dose/time)', critical: false },
